...
import { enable, getAccount, deposit, withdraw } from '@/utils/web3Utils'
  ...
  depositAccount ({ state }, amount) {
    return new Promise((resolve, reject) => {
      if (!state.wallet_address) {
        ...
        deposit(state.wallet_address, amount).then(r => resolve(r)).catch(err => reject(err))
      }
    })
  },
  withdrawAccount ({ state }, amount) {
    return new Promise((resolve, reject) => {
      if (!state.wallet_address) {
        ...
        withdraw(state.wallet_address, amount).then(r => resolve(r)).catch(err => reject(err))
      }
    })
  }
}
