...
const actions = {
  ...
  openPosition({ state, dispatch }, { side, quantityType, size, openType, price, leverage, brokerId }) {
    return new Promise((resolve, reject) => {
      ...
      const params = {
        token: token.address, side, openType, size: size.toString(), price, leverage, quantityType
      }
  ...
  loadHomeData({ state, commit, dispatch }, entrustType = 0) {
    ...
    return (async function () {
      ...
      data.traderOpenUpperBound = await contract.getTraderOpenUpperBound({
        token: curPair.address, trader: state.wallet_address
  ...
  loadAccountData({ state, commit }) {
    ...
    return (async function () {
      ...
      Object.assign(accountData, {
        marginBalance: tradeVariables.marginBalance
  ...
  getTraderOpenUpperBound({ state, commit }, { openType, price, leverage }) {
    return (async () => {
      ...
      const data = await contract.getTraderOpenUpperBound({
        token: token
